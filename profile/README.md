...
### Find the missing unit

Two energy ledgers sit in this repository - 4,096 integer transfers across 64 accounts each, generated by [the same program](tools/conservation_challenge.py). In one of them, a single entry was altered by **one unit** - one part in billions of total flow. No chart will show it. No statistic will find it fast. The books find it instantly, because conservation is a sum, not a vibe:

```bash
git clone https://github.com/coherence-energy-labs/.github && cd .github
python tools/conservation_challenge.py --audit assets/ledger-a.json   # one says BOOKS CLOSE, drift = 0
python tools/conservation_challenge.py --audit assets/ledger-b.json   # the other names the broken accounts
```

The forged ledger doesn't just fail - the audit names the exact accounts where conservation breaks, drift `±1`, at both ends of the altered transfer. This is the discipline our applied systems run on: exact-integer books that close to zero, or say precisely why they don't.
