...
LumOne deserves a real hearing because on **ceiling alone it beats Obsign.** "An LLM that answers only when it can back the answer with a machine-checkable warrant, and signs a certificate of what it cannot know instead of bluffing" targets *the* defining failure of applied AI — hallucination / unknown-unknowns — and gates every regulated LLM deployment on earth. It has a genuinely publishable measured result, and the current best number is stronger than the first one this report cited (see §10, Round 4): after LumOne's own adversarial audit found its baseline scoring had *undercounted legitimate declines* and rescored all 306 generations, the corrected headline is **baseline false answers 76.9% vs LumOne 0.93%**, still CI-separated [−0.84, −0.68] (`BUILD_LOG.md:511`) — with out-of-distribution transfer on SelfAware of **0.787 → 0.173** (`BUILD_LOG.md:583`) and an honest note that confidence calibration did not transfer as well. Its 21 solver lanes (exact arithmetic, Z3 over `int64`, SymPy CAS, exhaustive logic) are real and run *before* the model. And its **Provable Ignorance Certificate** — a signed, bounded, third-party-re-runnable certificate that a complete search found neither proof nor refutation, pinned to a hash of the belief state and composable across independent agents — is the single most novel primitive in the entire AI cluster. It has essentially no prior art.
...
**Round 4 — the LumOne verification round (owner's review, confirmed against source).** The owner re-read the LumOne source under the Round-2 objective and returned scientific corrections; every one was verified against the repo before adoption:

- **The evidence base is stronger than the number this report first cited.** The 87%→3.7% figure (`data/m1_report_14b.json`, n=199) is superseded: LumOne's own audit found the hedge lexicon *undercounted legitimate baseline declines* — an evaluation bias favoring itself — extended the lexicon, rescored all 306 generations offline (10 hedge flips, 0 correctness flips), and reduced its own headline. **Corrected: baseline false answers 76.9% (was 84.3) vs LumOne 0.93% (was 1.85), CI-separated [−0.84, −0.68], all M1 GO checks holding on corrected data** (`BUILD_LOG.md:511`). A project that catches a bias in its own favor and publishes the weaker number is producing exactly the kind of evidence this whole analysis prizes. Out-of-distribution: SelfAware unanswerable false-answer rate **0.787 → 0.173** (~4.5×, "a benchmark it never saw," `BUILD_LOG.md:583`), abstention routing 25.3% vs 79.3% answered-when-unanswerable, with the honest caveat that confidence calibration did not transfer (`BUILD_LOG.md:280`). And the frontier has moved past the 42.6% cited earlier: **R0 CERTIFIED (independent judge, coverage 1.0); R1 on-trial 63%, verified error 0.0%, wrong@high 0, fabricated 0 — "R1 sound; coverage is the climb"** (`BUILD_ROADMAP.md:5–7`), alongside the repo's own statement that LumOne is "not yet an evidenced AGI by the honest bar." The scientific scoreboard that matters is exactly that pair: **coverage climbing while verified error holds at ≈0.**
- **The defensible phrasing of the PIC — and the code already uses it.** Never "the AI proves it doesn't know." The provable statement is: *a specified deterministic search over a cryptographically committed belief state, under specified rules and bounds, found neither proof nor refutation* — a **bounded ignorance certificate**, `Unknown(q | K, R, d, b)`. Verified detail that makes this correction easy: the code's own vocabulary is already correct — `pic.py` mints only on `UNKNOWN` within declared bounds, binds the organ root via `biography.attest`, and the federation verdict field is literally **`"unknown_under_bounds"`** (`ignorance_algebra.py:59`). The overclaim exists only in the marketing sentence; the fix is to promote the code's own language to the public claim. Bounded is not a weakness — it is what makes the statement provable, and it pre-answers the obvious attack ("the fact was one hop outside your bound" — yes, and the certificate says exactly which bound).
- **The Ignorance Algebra's real gap: distinct signer ≠ independent mind.** Confirmed at code level: `federate()` requires `need` distinct **public keys** (`signers = {pubkey}`, `ignorance_algebra.py:64`) and intersects bounds to the weakest shared box — but carries no commitments to model lineage, retrieval-corpus roots, rule-set or software hashes. Two keys can share a corpus, a base model, or an operator; the docstring says "N INDEPENDENT minds" while the code proves N distinct keys. The moonshot therefore upgrades to a **Provenance-Disjoint Ignorance Algebra**: each PIC carries commitments to belief-state root, model lineage, corpus roots, rule-set hash, bounds, and software versions, so a federation distinguishes *two signatures* from *two genuinely different epistemic searches*. Two estate-native reinforcements: (1) **council already fixed this exact bug class** — its Invariant-4 self-audit found two providers running the same model counted as two witnesses, and fixed it by merging providers into connected components under any correlation signal, with a calibration test proving the merge doesn't collapse genuinely distinct minds; lift that mechanism, replacing free-text labels with the cryptographic commitments. (2) **The disjointness evidence must itself carry an assurance level** — model-lineage commitments are attestations today, not re-derivable — so a federated PIC should carry two covenant levels (search soundness, independence soundness) composed by the covenant's own law, `ACHIEVED = min(CLAIMED, REVERIFIABLE)`. Otherwise the federation manufactures exactly the unlabeled-weakest-link the estate exists to prevent.
- **Novelty claim narrowed.** "Zero prior art anywhere" is retracted. Adjacent art is real: LRAT-certified UNSAT (this estate ships it in ONE-Q), bounded model checking (Corral/Z3), cryptographic "proofs of ignorance" (a different object — absence of an NP witness), selective prediction/abstention with statistical guarantees, plus closed-world negation-as-failure and Certificate-Transparency proofs of absence (this audit's own analogs). The defensible, paper-worthy claim: **a portable certificate binding an AI's bounded inability to settle a semantic claim to a cryptographically committed knowledge state and deterministic reasoning procedure, composable across provenance-disjoint agents.** In one line: negation-as-failure, made portable and accountable.
- **"Finished LumOne" is one program, six commitments:** (1) epistemic output first-class — no free-form answer escapes without a machine-readable state (PROVED / REFUTED / KNOWN / BOUNDED-UNKNOWN / UNVERIFIED), the PIC wired into the product path; (2) the **Lane Factory**, publishing one curve — certified coverage vs verified error; (3) stranger-native verification (the Obsign property: answer + warrant + state commitment, checkable without producer code); (4) a question-native planner (the VMP principle lifted up: cheapest lawful sequence of checks that settles the claim, with evidence that stopping was valid); (5) provenance-disjoint federation; (6) the frontier as the scoreboard: **C(t) = P(claim mechanically settled) subject to P(false | certified) ≤ ε.** Two disciplines keep that metric honest: the denominator must be a **pre-registered, adversarially-sampled claim distribution the lanes never train on** (the SelfAware pattern — "pure transfer," `BUILD_LOG.md:538` — generalized to a growing benchmark family, hash-pinned), reported per-stratum with the worst stratum alongside the mean; and "mechanically consequential" gets formalized as **register monotonicity** — no surface may present a stronger register than the epistemic state licenses — enforced structurally, the way the repo already enforces "a declined answer can never wear a confident register" (`BUILD_LOG.md:757`) and its AST-level deadline invariant.

If C(t) climbs toward 90% while ε stays near zero, the project is profound; if it stalls at 20–30%, the experiment has said something equally important. Either way the sentence the program is built around is the owner's: **"A machine should never merely answer. It should establish what it is entitled to claim, show why, and certify the boundary where its knowledge ends."**

**Three corrections adopted from the owner's review (Round 3):**
